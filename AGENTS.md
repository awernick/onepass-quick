...
fastlane/
  Fastfile                    # Build, install, clean lanes
OnePassQuick/
  OnePassQuick.xcodeproj/     # Generated by XcodeGen (do not edit directly)
  OnePassQuick/
    App/
      AppDelegate.swift       # App lifecycle, menu bar setup, no dock icon
      main.swift              # Entry point
      Preferences.swift       # UserDefaults-backed app preferences
    Panel/
      QuickAccessPanel.swift  # NSPanel subclass (floating window level)
      PanelController.swift   # Show/hide logic, positioning, actions
    Hotkey/
    ...
    Search/
      ...
      FuzzyMatcher.swift       # Fuzzy string matching with scoring
      SearchViewModel.swift    # Search state, filtering, Combine pipeline
    Views/
      SearchView.swift         # SwiftUI: search field + results list + toast
      SearchField.swift        # NSViewRepresentable: NSTextField wrapper
      ItemRow.swift            # SwiftUI: single result row with highlights
      ShortcutHintsBar.swift   # SwiftUI: keyboard shortcut hints footer
    Actions/
...
The Xcode project is generated from `project.yml` via XcodeGen. **Do not
edit `.xcodeproj` directly** -- it will be overwritten.

```sh
# Preferred: build, install to /Applications, and launch
fastlane mac install

# Build only (no install)
fastlane mac build

# Build and launch from DerivedData
fastlane mac launch

# Clean build artifacts
fastlane mac clean

# Regenerate .xcodeproj only
fastlane mac generate
```

No third-party Swift dependencies. No CocoaPods, SPM packages, or Carthage.
...
| Fastlane | Build automation (`mac install`, `mac build`, etc.) |

### QA Before Commit

After completing a feature or fix, **do not commit immediately**. Human QA
is non-negotiable for UI work:
...
> is defined in the global `~/.config/Claude/AGENTS.md`.
